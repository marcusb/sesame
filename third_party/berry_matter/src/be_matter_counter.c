...
typedef struct {
  ...
  uint32_t window;      // window of received messages (bitset 32)
} matter_counter_t;
...
static void mc_randomize(matter_counter_t *c) {
  ...
  xApplicationGetRandomNumber(&c->counter);
...
int32_t mc_validate(matter_counter_t *c, uint32_t new_val, bbool strict_mode) {
  ...
  if (new_val > c->counter) {   // counter is in the future, same handling for both modes
    if (new_val - c->counter > MESSAGE_COUNTER_WINDOW_SIZE) {
      c->window = 0;    // no index is in the window anymore, clear all
    } else {
      c->window <<= new_val - c->counter;
      c->window |= (1U << (new_val - c->counter - 1));   // mark previous val
      // adjust window
    }
    c->counter = new_val;
    return btrue;
    ...
    if (gap <= 32) {
      if (c->window & (1U << (gap-1))) {
        return bfalse;         // reject because already seen
      } else {
        c->window |= (1U << (gap-1));
        return btrue;
      }
...
static int mc_tostring(bvm *vm) {
  ...
  if (c->counter != 0) {
    ...
    for (uint32_t i = MESSAGE_COUNTER_WINDOW_SIZE; i > 0; i--) {
      be_pushint(vm, (c->window >> (i-1)) & 0x1);
