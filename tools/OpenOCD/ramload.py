...
def main():
    ...
    try:
        opts, args = getopt.gnu_getopt(
            sys.argv[1:], "i:sh", ["interface=", "semihosting", "help"]
        )
        if len(args) != 1:
            print_usage()
            sys.exit()
    ...
    if not len(READELF):
        ...
        if not len(READELF):
            print(
                "Error: readelf utility not found, please install binutils or appropriate package"
            )
            sys.exit()
    ...
    if (int(entry_point, 16) & 0x1F000000) == 0x1F000000:
        print(
            "Error: Provided firmware image (.axf) is XIP enabled and hence ramload is not possible, please use flashprog to flash the image (.bin)"  # noqa: E501
        )
        sys.exit()
    ...
    if not len(NM):
        ...
        if not len(NM):
            print(
                "Error: nm utility not found, please install binutils or appropriate package"
            )
            sys.exit()
    ...
    for item in nm_output.split("\n"):
        if "_ol_ovl" in item:
            print(
                "Error: Provided firmware image (.axf) is overlays enabled and hence ramload is not possible, please use flashprog to flash the image."  # noqa: E501
            )
            sys.exit()
