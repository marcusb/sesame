...
def copy_config_option(comp_name, comp_file):
    if (comp_name != "debug") and (os.path.exists(file_path(comp_file)) is False):
    ...
    try:
        ...
        try:
            if (int(subargs[0], 0) < 0) or (int(subargs[0], 0) >= MAX_FLASH_TYPE):
                raise WrongArgs(
                    "Flash id is out of range [0-" + (str)(MAX_FLASH_TYPE - 1) + "]"
                )
            ...
            if mfg_cmd == "write":
                if os.path.exists(file_path(subargs[2])) is False:
        ...
        if mfg_cmd == "read":
            ...
            line = (
                "write " + subargs[0] + " " + subargs[1] + " " + file_path(subargs[2])
            )
        elif mfg_cmd == "erase":
...
def run_flashprog(chip_id, interactive):
    ...
    readelf_output = subprocess.Popen(
        [READELF, flashprog_axf, "-h"], stdout=subprocess.PIPE
    )
    ...
    if interactive:
        p = subprocess.Popen(
            [
                ...
                "init",
                "-c",
                "sh_load " + flashprog_axf + " " + entry_point,
            ]
        )
        p.communicate()
    else:
...
def main():
    ...
    try:
        opts, args = getopt.gnu_getopt(
            sys.argv[1:], non_comp_shortopts, non_comp_longopts + comp_longopts
        )
        if len(args):
            exit()
    ...
    if (
        ...
        or (os.path.exists(LAYOUT_FILE))
        or (os.path.exists(MFG_FILE))
        or interactive
    ):
        chip_id = get_chip_id()
