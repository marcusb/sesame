...
int mbedtls_ccm_setkey(mbedtls_ccm_context* ctx, mbedtls_cipher_id_t cipher,
    ...
    if (cipher_info == NULL) return (MBEDTLS_ERR_CCM_BAD_INPUT);

    if (cipher_info->block_size != 16) return (MBEDTLS_ERR_CCM_BAD_INPUT);
    ...
    do {                                                               \
        if ((ret = mbedtls_cipher_update(&ctx->cipher_ctx, ctr, 16, b, \
            ...
            return (ret);                                              \
                                                                       ...
                                                                       \
        for (i = 0; i < (len); i++) (dst)[i] = (src)[i] ^ b[i];        \
    } while (0)
 ...
 */
/* CCM selftest fails on ARM Cortex M with IAR 8.11 with common subexpression
 * elimination optimalization enabled */
...
static int ccm_auth_crypt(mbedtls_ccm_context* ctx, int mode, size_t length,
    ...
    if (add_len > 0xFF00) return (MBEDTLS_ERR_CCM_BAD_INPUT);
    ...
    if (len_left > 0) return (MBEDTLS_ERR_CCM_BAD_INPUT);
...
int mbedtls_ccm_star_encrypt_and_tag(mbedtls_ccm_context* ctx, size_t length,
    ...
    if ((tag_len == 8) || (tag_len == 4) || (tag_len == 16)) {
        status = AES_EncryptTagCcmWithLock(
            KSDK_MBEDTLS_AES_INSTANCE, ctx->key, ctx->keybytes, input, output,
            length, iv, iv_len, add, add_len, tag, tag_len);
...
int mbedtls_ccm_star_auth_decrypt(mbedtls_ccm_context* ctx, size_t length,
    ...
    if ((tag_len == 8) || (tag_len == 4) || (tag_len == 16)) {
        status = AES_DecryptTagCcmWithLock(
            KSDK_MBEDTLS_AES_INSTANCE, ctx->key, ctx->keybytes, input, output,
            length, iv, iv_len, add, add_len, tag, tag_len);
