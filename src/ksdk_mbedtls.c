...
static int mbedtls_entropy_remove_source(
    mbedtls_entropy_context* ctx, mbedtls_entropy_f_source_ptr f_source) {
    int i;
    int ret = -1;
...
static int internal_entropy_ctr_drbg_setup(void) {
    ...
    if (!s_internalEntropyContextSet) {
        ...
        mbedtls_entropy_remove_source(&s_internalMbedtlsGdata.entropy,
        ...
        mbedtls_ctr_drbg_set_reseed_interval(&s_internalMbedtlsGdata.ctrDrbg,
                                             INT_MAX);
        mbedtls_ctr_drbg_set_prediction_resistance(
            &s_internalMbedtlsGdata.ctrDrbg, MBEDTLS_CTR_DRBG_PR_OFF);
