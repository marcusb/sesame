...
def httpd_harness() -> Iterator[Harness]:
    # Pass guest_port=80 to enable hostfwd in QEMU
    gen = _run_harness("httpd", guest_port=80)
    harness = next(gen)
    ...
    while time.monotonic() < deadline:
        try:
            ...
            print(f"HTTPD poll error: {e}")
            time.sleep(0.1)

    else:
        pytest.fail("httpd did not become ready on host port")
