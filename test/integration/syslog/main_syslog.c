...
static void on_cmd(const char* line) {
    /*
     * Commands understood by syslog_it:
     *   cfg HOST PORT            configure_logging() → register log_syslog
     * backend log LEVEL MSG            emit a log record at LEVEL
     *
    ...
    if (strncmp(line, "cfg ", 4) == 0) {
        ...
        if (strcmp(level, "error") == 0)
            vLoggingPrintfError("%s", msg);
        else if (strcmp(level, "warn") == 0)
            vLoggingPrintfWarn("%s", msg);
        else if (strcmp(level, "info") == 0)
            vLoggingPrintfInfo("%s", msg);
        else if (strcmp(level, "debug") == 0)
            vLoggingPrintfDebug("%s", msg);
    }
}
